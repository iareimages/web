...
async function fetchStudentData() {
    try {
        const admissionsRef = ref(db, 'admissionsdata');
    ...
    if (user) {
        ...
        if (hasAccess) {
            ...
            johnSection.innerHTML = `
                        ...
                        <center>
                            <div class="ddess">
                                <h3 id="constructionMessage">Sorry.... <br><br>Website Under Construction.<strong></strong></h3>
                            </div>
                        </center>`;
        }
    } else {
...
async function signInWithGoogle() {
    try {
        ...
        // Fetch student data after successful sign-in
        await fetchStudentData();
...
async function displayUserInfo(email) {
    ...
    // Get active filters from the global scope (set by bsdedd.js)
    const currentFilters = window.activeFilters || [];

    // Format filters for storage
    // 'selected': Comma-separated headers (e.g., "Student Name, Branch")
    // 'entered': Comma-separated values (e.g., "John, CSE")
    const selectedHeaders = currentFilters.map(f => f.headerName).join(', ');
    const enteredValues = currentFilters.map(f => f.searchValue).join(', ');
